...
public class Enemymovement : Toolbox
{
    ...
    _TrackPlayerInfo Increasemon = new _TrackPlayerInfo();
    ...
    void onstart()
    {
        health = 1f;
        moveSpeed = .8f;
        SpawnPositionx = transform.position.x + 2f;
        SpawnPositiony = transform.position.y + 2f;
    ...
    void move() 
    {
        ...
        if (health <= 0.01f)
        {
            Increasemon._totalmoney(1);
            Destroy(gameObject);
        }
        if (ifclose(1, "Commander"))
        {
